  ...
  describe('when supplied a basic config', function () {
    ...
    describe('pub sub', function () {
      describe('when adding a custom event handler', function () {
        it('runs an custom function when a tracked event is passed in the logs.', function (done) {
          var funcObj = {
            func: function () {
            }
          };
          sinon.spy(funcObj, "func")

          var log = winWithLogs(config);
          log.addEventHandler("test", funcObj.func)
          log.log("@test").then(
            function () {
              expect(funcObj.func).to.have.been.called;
              done();
            }
          )
        });
